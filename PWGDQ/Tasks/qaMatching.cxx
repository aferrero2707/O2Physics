...
DECLARE_SOA_COLUMN(Bc, bc, int64_t);
...
struct QaMatching {
  ...
  Preslice<FwdTracksReAlign> muonsReAlignPerCollisions = aod::fwdtrack::collisionId;
  ...
  Configurable<bool> cfgMatchingRequireMid{"cfgMatchingRequireMid", false, ""};
  ...
  struct MchTrackInfo : public TrackTimeInfo {
    int64_t index{-1};
  ...
  struct CollisionInfo {
    ...
    std::unordered_map<int64_t, TrackTimeInfo> mftTimeInfos;
    // vector of MCH(-MID) track indexes
    std::map<int64_t, MchTrackInfo> mchTracks;
    // mapping between MCH tracks and time-compatible MFT tracks
    //std::unordered_map<int64_t, std::vector<int64_t>> mchCompatMftTracks;
  ...
  int getMftMchMatchAttempts(MchTrackInfo& mchTrackInfo,
                             const std::unordered_map<int64_t, TrackTimeInfo>& mftTracksInfos,
                             bool verbose = false)
  {
    const auto& bcMch = mchTrackInfo.bc;
    if (verbose) {
      std::cout << std::format("[PIPPO] MCH BC: {} + {} = {}",
          bcMch,
          mchTrackInfo.time / o2::constants::lhc::LHCBunchSpacingNS,
          bcMch + mchTrackInfo.time / o2::constants::lhc::LHCBunchSpacingNS) << std::endl;
    }

    int attempts{0};
    for (const auto& [mftTrackIndex, mftTrackInfo] : mftTracksInfos) {
      const auto& bcMft = mftTrackInfo.bc;
      ...
      attempts += 1;
      mchTrackInfo.compatMftTracks.push_back(mftTrackIndex);
      if (verbose) {
                             ...
                             bool verbose = false)
  {
    ...
    for (const auto& mftTrack : mftTracks) {
      ...
      attempts += 1;
      compatMftTracks.push_back(mftTrack.globalIndex());
      if (verbose) {
        std::cout << std::format("[PIPPO]   MFT track added, attempts={}", attempts) << std::endl;
      }
    }
                      ...
                      CollisionInfos& collisionInfos)
  {
    ...
    for (size_t cid = 0; cid < collisionIds.size(); cid++) {
      ...
      for (const auto& mftTrack : mftTracks) {
        ...
        auto& timeInfo = collisionInfo.mftTimeInfos[mftTrackIndex];
        timeInfo.bc = bc.globalBC();
        timeInfo.time = mftTrack.trackTime();
        timeInfo.timeRes = mftTrack.trackTimeRes();
      }
      ...
      for (const auto& muonTrack : muonTracks) {
        ...
        if (static_cast<int>(muonTrack.trackType()) > GlobalTrackTypeMax) {
          ...
          //getMftMchMatchAttempts(collisions, bcs, muonTrack, mftTracks, mchTrackInfo.compatMftTracks);
          ...
          //std::cout << std::format("Propagating tracks {}, {} from candidate {}", mchTrackIndex, mftTrack.globalIndex(), muonTrackIndex) << std::endl;
          ...
          //std::cout << std::format("After propagating tracks {}, {} from candidate {}", mchTrackIndex, mftTrack.globalIndex(), muonTrackIndex) << std::endl;
    ...
    for (auto& [collisionIndex, collisionInfo] : collisionInfos) { // o2-linter: disable=const-ref-in-for-loop (object is modified in loop)
      ...
      for (auto& [mchIndex, globalTracksVector] : collisionInfo.matchingCandidates) { // o2-linter: disable=const-ref-in-for-loop (object is modified in loop)
        ...
        //std::cout << std::format("Got Muon track {} with {} candidates", mchTrack.globalIndex(), globalTracksVector.size()) << std::endl;
        int ranking = 1;
        for (auto& candidate : globalTracksVector) { // o2-linter: disable=const-ref-in-for-loop (object is modified in loop)
          ...
          //std::cout << std::format("Got Muon candidate {}/{}", candidate.muonTrackId, candidate.mftTrackId) << std::endl;
          if constexpr (isMC) {
        ...
        if (isPaired && trueMatchIndex == 0 && mftMchMatchAttempts < 20) {
          ...
          getMftMchMatchAttempts(collisions, bcs, mchTrack, mftTracks, attempts, true);
        }
      }
    }
  }
                         ...
                         MatchingPlotter* plotter)
  {
    ...
    for (const auto& [mchIndex, globalTracksVector] : matchingCandidates) {
      ...
      for (const auto& candidate : globalTracksVector) {
        ...
        //std::cout << std::format("Candidate chi2: {}", candidate.matchChi2) << std::endl;
  ...
  void fillMatchingPlotsMc(C const& collision,
                           ...
                           bool /*verbose*/ = false)
  {
    static std::ofstream matchingOut("matching-qa-2.txt");
    ...
    for (const auto& [mchIndex, globalTracksVector] : matchingCandidates) {
      ...
      for (const auto& candidate : globalTracksVector) {
        ...
        candidateIndex += 1;
        matchingOut << std::format("Candidate bc={} mch={} chi2={:0.3f} p={:0.3f}", collisionInfo.bc, mchIndex, candidate.matchChi2, mchTrack.p()) << std::endl;
      }
    }
  ...
  void runMlMatching(C const& collisions,
                     ...
                     MatchingCandidates& newMatchingCandidates)
  {
    static std::ofstream mlout("mlout.cvs");
    //static auto& mlout = std::cout;
    static int trackId = 0;

    int modelId = -1;
    for (int id = 0; id < MlModelsNum; id++) {
      if (label == configMlOptions.modelLabels[id]->value) {
        modelId = id;
        break;
      }
    }

    if (false && modelId >= 0 && trackId == 0) {
      auto inputFeatures = configMlOptions.inputFeatures[modelId]->value;
      mlout << "   ,";
      for (int i = 0; i < inputFeatures.size(); i++) {
        mlout << std::format("{:>16},", inputFeatures[i]);
      }
      mlout << std::endl;
    }
    ...
    for (const auto& [mchIndex, globalTracksVector] : matchingCandidates) {
      ...
      for (const auto& candidate : globalTracksVector) {
        ...
        if (modelId >= 0 && trackId < 100) {
          auto inputFeatures = configMlOptions.inputFeatures[modelId]->value;
          if (inputFeatures.size() == inputML.size()) {
            ...
            std::cout << std::format("Features size mismatch: {} / {}", inputFeatures.size(), inputML.size()) << std::endl;
          }
        }
        trackId += 1;
  ...
  template <bool isMC, class C, class BC, class TMUON, class TMFT, class CMFT>
  void runChi2MatchingAllTracks(const CollisionInfo& collisionInfo,
                                C const& collisions,
                                BC const& bcs,
                                TMUON const& muonTracks,
                                TMFT const& mftTracks,
                                ...
                                MatchingCandidates& newMatchingCandidates)
  {
    ...
    for (const auto& mchTrackInfo : collisionInfo.mchTracks) {
      auto mchIndex = mchTrackInfo.second.index;
      const auto& muonTrack = muonTracks.rawIteratorAt(mchIndex);
      if (!muonTrack.has_collision()) {
        continue;
      }
      ...
      for (const auto& [mftIndex, mftCovIndex] : mftTrackCovs) {
        ...
        //std::cout << std::format("Match candidate {} {}/{} {}/{}:\n  track IDs: {} {}, match chi2: {:0.3f}\n  glob track ID: {}, match chi2: {:0.3f}",
        //    newMatchingCandidates[mchIndex].size(), id1, muonTracks.size(), id2, mftTrackCovs.size(),
        //    mchIndex, mftIndex, matchChi2, globalTrackIndex, matchChi2Prod) << std::endl;
        //}
      }
    }
                                ...
                                MatchingCandidates& newMatchingCandidates)
  {
    ...
    runChi2MatchingAllTracks<isMC>(collisionInfo, collisions, bcs, muonTracks, mftTracks, mftCovs, funcName, matchingPlaneZ, extrapMethod, matchablePairs, matchingCandidates, newMatchingCandidates);
  }

  template <bool isMC, class C, class BC, class TMUON, class TMFT, class CMFT>
  void runMlMatchingAllTracks(const CollisionInfo& collisionInfo,
                              C const& collisions,
                              BC const& bcs,
                              TMUON const& muonTracks,
                              TMFT const& mftTracks,
                              ...
                              MatchingCandidates& newMatchingCandidates)
  {
    ...
    for (const auto& mchTrackInfo : collisionInfo.mchTracks) {
      ...
      for (const auto& [mftIndex, mftCovIndex] : mftTrackCovs) {
        ...
        //}
        float matchScore = (output[0] != 0) ? 1. / output[0] : 0;
                        ...
                        CMFT const& mftCovs)
  {
    ...
    fillQaMatchingAodEventForCollision(collisionInfo, collision, bc, collisionInfo.reducedEventId, debugCounter);
    ...
    fillQaMatchingAodTablesForCollision(collision, bc, muonTracks, mftTracks, collisionInfo.matchingCandidates, matchingMethodCounter, collisionInfo.reducedEventId, collisionInfo.reducedMchTrackIds);
    if constexpr (isMC) {
    ...
    for (const auto& [label, func] : matchingChi2Functions) {
      ...
      if (cfgMatchAllTracks.value) {
        runChi2MatchingAllTracks<isMC>(collisionInfo, collisions, bcs, muonTracks, mftTracks, mftCovs, label, collisionInfo.matchablePairs, collisionInfo.matchingCandidates, matchingCandidates);
      } else {
      ...
      fillQaMatchingAodTablesForCollision(collision, bc, muonTracks, mftTracks, matchingCandidates, matchingMethodCounter, collisionInfo.reducedEventId, collisionInfo.reducedMchTrackIds);
      if constexpr (isMC) {
    ...
    for (const auto& [label, mlResponse] : matchingMlResponses) {
      MatchingCandidates matchingCandidates;
      if (cfgMatchAllTracks.value) {
        runMlMatchingAllTracks<isMC>(collisionInfo, collisions, bcs, muonTracks, mftTracks, mftCovs, label, collisionInfo.matchablePairs, collisionInfo.matchingCandidates, matchingCandidates);
      } else {
      ...
      std::cout << "Filling histograms for ML matching - begin" << std::endl;
      matchingMethodCounter += 1;
      fillQaMatchingAodTablesForCollision(collision, bc, muonTracks, mftTracks, matchingCandidates, matchingMethodCounter, collisionInfo.reducedEventId, collisionInfo.reducedMchTrackIds);
      if constexpr (isMC) {
      ...
      std::cout << "Filling histograms for ML matching - end" << std::endl;
    }
  ...
  void fillQaMatchingAodTablesForCollision(TCOLLISION const& collision,
                                           ...
                                           std::map<int64_t, int64_t> reducedMchTrackIds)
  {
    static std::ofstream matchingOut("matching-qa.txt");
    ...
    for (const auto& [mchIndex, candidates] : matchingCandidates) {
      ...
      for (const auto& candidate : candidates) {
        ...
        if (matchLabel == 0) {
          matchingOut << std::format("Candidate bc={} mch={} chi2={:0.3f} p={:0.3f}", bc.globalBC(), reducedMchTrackId, candidate.matchChi2, mchTrack.p()) << std::endl;
        }
  ...
  template <class TCOLLISION, class TBC>
  void fillQaMatchingAodEventForCollision(const CollisionInfo& collisionInfo,
                                          ...
                                          int& debugCounter)
  {
    //matchingOut << std::format("Collision {}", collision.globalIndex()) << std::endl;

    int32_t mftMultiplicity = static_cast<int32_t>(collisionInfo.mftTracks.size());
    qaMatchingEvents(
      static_cast<int64_t>(bc.globalBC()),
      mftMultiplicity,
                                           ...
                                           int32_t reducedEventId)
  {
    ...
    for (const auto& mchTrackInfo : collisionInfo.mchTracks) {
      if (std::find(mchIds.begin(), mchIds.end(), mchTrackInfo.second.index) == mchIds.end()) {
        mchIds.emplace_back(mchTrackInfo.second.index);
      }
    }
    ...
    for (const auto& mchIndex : mchIds) {
      ...
      //matchingOut << std::format("  Muon {} with p={:0.3f}", mchIndex, mchTrack.p()) << std::endl;
                 ...
                 MyMFTCovariances const& mftCovs)
  {
    ...
    std::cout << std::format("muonTracks.size(): {}", muonTracks.size()) << std::endl;
                        ...
                        MyMFTCovariances const& mftCovs)
  {
    ...
    for (const auto& collision : collisions) {
      auto tracksThisCollision = muonTracks.sliceBy(muonsReAlignPerCollisions, collision.globalIndex());
    }
