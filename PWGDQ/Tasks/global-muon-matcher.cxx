...
struct GlobalMuonMatching {
  ...
  void initMatchingFunctions()
  {
    ...
    // correct the MFT value to be in the [-pi, pi] range
    auto getCorrectedPhi = [](const o2::track::TrackParCovFwd& mftTrack) -> float {
      float phi = mftTrack.getPhi();
      static constexpr float pi = TMath::Pi();
      static constexpr float twopi = pi;
      while (phi > pi) {
        phi -= twopi;
    ...
    mMatchingFunctionMap["matchALL"] = [&](const o2::track::TrackParCovFwd& mchTrack, const o2::track::TrackParCovFwd& mftTrack) -> std::tuple<double, int> {
      ...
      SMatrix55Sym hK, vK;
      SVector5 mK(mftTrack.getX(), mftTrack.getY(), getCorrectedPhi(mftTrack),
